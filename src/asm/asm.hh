...
public:
  ...
  RegisterOffsetAsmOpd(std::shared_ptr<Register> _reg, int _offset)
...
public:
  ...
  ComputeAsmStmt(std::shared_ptr<AsmOpd> _lhs, std::shared_ptr<AsmOpd> _rhs1,
                 Operator _op, std::shared_ptr<AsmOpd> _rhs2)
      : lhs(_lhs), rhs1(_rhs1), op(_op), rhs2(_rhs2) {
    if (lhs)
...
public:
  ...
  MoveAsmStmt(MoveType mt, std::shared_ptr<AsmOpd> lhs,
              std::shared_ptr<AsmOpd> rhs1,
              std::shared_ptr<AsmOpd> rhs2 = nullptr);
...
public:
  ...
  LabelAsmStmt(std::shared_ptr<AsmOpd> _label)
      : label(std::dynamic_pointer_cast<LabelAsmOpd>(_label)) {}
  void print(std::ostream &os) const override;
};
...
public:
  std::shared_ptr<LabelAsmOpd> label;
  GotoAsmStmt(std::shared_ptr<AsmOpd> _label)
      : label(std::dynamic_pointer_cast<LabelAsmOpd>(_label)) {}
  void print(std::ostream &os) const override;
};
...
public:
  ...
  IfGotoAsmStmt(std::shared_ptr<AsmOpd> _label, std::shared_ptr<AsmOpd> _var)
      : label(std::dynamic_pointer_cast<LabelAsmOpd>(_label)), var(_var) {}
...
public:
  ...
  JumpRegAsmStmt() {
    reg = std::make_shared<RegisterAsmOpd>(
        std::make_shared<SpecialRegister>(specialRegType::ra));
  }
